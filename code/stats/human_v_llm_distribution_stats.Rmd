...
df_results$modality <- factor(df_results$modality, levels = c("video", "audio", "text"))
...
modality_contrast <- contr.poly(length(unique(df_results$modality))) #c(1,-1)
contrasts(df_results$modality, how.many=2) <- cbind(modality_contrast)
...
kl_divergence <- 'kl_divergence ~ modality + (1|word_index:task) + (1|model_name)'

model <- lmer(kl_divergence, data = df_cleaned)

tab_model(model, show.est = TRUE, show.se = TRUE, show.df = TRUE, show.stat = TRUE)

plot_model(model, type='pred',  terms=c('modality' ))
```
...
df_cleaned_filtered <- remove_outliers(df_results_filtered, 'log_model_predictability')

model <- glmmTMB(
  formula = human_predictability ~ modality * log_model_predictability  + (1|word_index:task) + (1|model_name),
  data = df_cleaned_filtered,
  family = gaussian(link='log')
)
