...
predictability_formula <- 'human_predictability ~ modality * log_model_predictability * prominence_mean + (1|word_index:task)'

model <- glmer(predictability_formula, data = df_cleaned,
               family=gaussian(link='log'))
...
model <- lmer(kl_divergence, data = df_results)
