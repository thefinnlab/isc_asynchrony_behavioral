...
contrasts(df_results$model_type, how.many=2) <- contr.poly(length(unique(df_results$model_type)))
...
plot_model(model, type='pred', terms=c('prominence_mean', 'model_type'))
...
# set order of variables
df_accuracy_contrast$modality <- factor(df_accuracy_contrast$modality, levels = c("video", "audio", "text"))
...
modality_contrast <- contr.poly(length(unique(df_accuracy_contrast$modality))) #c(1,-1)
contrasts(df_accuracy_contrast$modality, how.many=2) <- cbind(modality_contrast)
...
plot_model(model, type="pred", terms=c("model_entropy", "modality"))
...
plot_model(model, type="pred", terms=c("model_entropy", "modality", "prominence_mean"))
