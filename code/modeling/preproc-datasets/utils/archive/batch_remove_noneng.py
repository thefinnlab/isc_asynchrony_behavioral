...
if __name__ == "__main__":
    ...
    all_cmds = []
    script_fn = os.path.join(os.getcwd(), f'{dataset}_reorganize_dataset.py')
    job_num = 0
    ...
    for shard in range(N_SHARDS):
        cmd = [
            f"{DSQ_MODULES.replace('dark_matter', 'prosody')} sleep {shard * 2}; ",
            f"python voxceleb2_remove_noneng.py --dataset {dataset} {model_types} --num_jobs={CPUS_PER_TASK} --current_shard {shard} --num_shards {N_SHARDS}", 
          ]
    ...
    joblist_fn = os.path.join(joblists_dir, f'dsq_{dataset}_remove_noneng.txt')

    with open(joblist_fn, 'w') as f:
        for cmd in all_cmds:
          f.write(f"{cmd}\n")

    dsq_base_string = f'{dataset}_remove_noneng_files'
