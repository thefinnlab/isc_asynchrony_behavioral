...
if __name__ == "__main__":
    ...
    base_dir = os.path.join(DATASETS_DIR, 'nlp-datasets/avspeech/src')
    ...
    cmd = [
        ...
        f"python organize_avspeech.py --base_dir {base_dir} --move_files {p.move_files} --organize_clips {p.organize_clips}; ",
    ]
    ...
    joblist_fn = os.path.join(joblists_dir, f'dsq_organize_avspeech.txt')

    with open(joblist_fn, 'w') as f:
        for cmd in all_cmds:
            f.write(f"{cmd}\n")

    dsq_base_string = f'organize_avspeech'
