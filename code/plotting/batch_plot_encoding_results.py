...
if __name__ == "__main__":
	...
	TASKS = ['wheretheressmoke', 'howtodraw', 'odetostepfather']

	# now prep for the script
	all_cmds = []
	script_fn = os.path.join(os.getcwd(), 'plot_encoding_results.py')
	job_string = f'{DSQ_MODULES} srun python -u {script_fn}'
	job_num = 0

	for task, ses in product(TASKS, sessions):

		print (f'Preparing {task} {ses}')

		cmd = ''.join([
			f'{job_string} -d {p.dataset} -t {task} {ses} -m {MAIN_MODEL}']
		)

		all_cmds.append(cmd)
		job_num += 1

	joblist_fn = os.path.join(joblist_dir, f'{p.dataset}_plot_encoding_joblist.txt')

	with open(joblist_fn, 'w') as f:
		for cmd in all_cmds:
			f.write(f"{cmd}\n")
	
	dsq_base_string = f'dsq_plot_encoding'
