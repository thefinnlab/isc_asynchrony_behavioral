...
if __name__ == "__main__":
	...
	# set the train/test tasks
	session_tasks = {
		'ses-2': 
			['alternateithicatom', 'avatar'], #, 'legacy', 'odetostepfather', 'souls'],
		'ses-3':
			['howtodraw', 'myfirstdaywiththeyankees'], #, 'naked', 'undertheinfluence'],
		'ses-4':
		...
		'ses-6':
			['eyespy', 'hangtime'] #, 'itsabox', 'swimmingwithastronauts', 'thatthingonmyarm']
	}

	train_tasks = [v for k, v in session_tasks.items()]
	train_tasks = sum(train_tasks, [])
	train_tasks = ' '.join(train_tasks)

	# get each wheretheressmokerun as a test task
	test_tasks = [f"wheretheressmoke_run-{i + 1}" for i, k in enumerate(session_tasks.keys())]
	...
	for sub in sub_list:
		...
		if p.overwrite or not files_exist:
			...
			cmd = ''.join([
				f'{job_string} -d {p.dataset} -train {train_tasks} -test {test_tasks} -s {sub} -m {main_model} -i {str(iteration).zfill(5)} -f {features} -o {p.overwrite}']
			)
